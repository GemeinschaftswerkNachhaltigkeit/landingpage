  ...
  <div id="projekt-nachhaltigkeit" class="page">
    <section class="content-section breadcrumbs-section">
      <div class="breadcrumbs content-section-wrapper">
        ...
        <div class="current">
          {{ data.currentNav?.name }}
        </div>
      </div>
    </section>
    <section class="content-section page-hero">
      <content-with-image mobileSwapped :title="data.title" :tagline="data.tagline" :content="data.summary"
        :image="data.image" :alt="data.alt || ''">
        <div class="action">
          <div class="external-link">
            <LinkButton external variant="outline" :url="data.button_url">{{ data.button_text }}</LinkButton>
          </div>
        </div>
        <template v-slot:imageExt>
          <div class="project-logo">
            <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"
              viewBox="0 0 362.83 102.05">
              <defs>
    ...
    <section v-if="data.accordion_title && data.accordion_entries?.length" class="content-section">
      <div class="content-section-wrapper">
        <div class="accordion-title">
          <h2>{{ data.accordion_title }}</h2>
        </div>
        <accordion>
          <accordion-entry v-for="entry of data.accordion_entries" :key="entry.title" :title="entry.title"
            :subtitle="entry.subtitle" :content="entry.content" />
        </accordion>
        <div class="accordion-button">
          <LinkButton v-if="data.accordion_button_text && data.accordion_button_url" external
            :url="data.accordion_button_url">{{
              data.accordion_button_text }}</LinkButton>
        </div>
      </div>
    </section>
    <section class="content-section cards-section" v-if="orgas.content && orgas.content?.length">
      <div class="cards content-section-wrapper">
        <div class="title-wrapper">
          <h2 class="slider-title">{{ data.title_slider }}</h2>
        </div>

      </div>
    </section>
    <div class="carousel-wrapper" v-if="orgas && orgas?.length">
      <Carousel :items="orgas" :component="OrgaActivitySlide" framedCards></Carousel>
    </div>
...
const { data, pending, error, refresh } = await useAsyncData('projectN', async () => {
  const currentLocale = $i18n.locales.value.find(
    (i) => i.code === $i18n.locale.value
  );

  let projectNTranslations = [];
  let projectN = {};
  let parentNav = {}
  let currentNav = {}
  try {
    ...
    currentNav = parentNav?.submenu.find(s => s.target === 'projekt-nachhaltigkeit');

    projectN = await getSingletonItem({
      collection: "projekt_nachhaltigkeit"
    })
    projectNTranslations = await getItems({
      collection: "projekt_nachhaltigkeit_translations",
      params: {
        filter: {
