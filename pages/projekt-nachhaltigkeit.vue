<template>
  <div id="projekt-nachhaltigkeit" class="page">
    <section class="content-section breadcrumbs-section">
      <div class="breadcrumbs content-section-wrapper">
        <div>
          {{ data.parentNav?.name
          }}<span class="arrow">
            <svg
              width="8"
              height="12"
              viewBox="0 0 8 12"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M1.4 12L0 10.6L4.6 6L0 1.4L1.4 0L7.4 6L1.4 12Z"
                fill="#19486A"
              />
            </svg>
          </span>
        </div>

        <div class="current">
          {{ data.currentNav?.name }}
        </div>
      </div>
    </section>
    <section class="content-section page-hero">
      <content-with-image
        mobileSwapped
        :title="data.title"
        :tagline="data.tagline"
        :content="data.summary"
        :image="data.image"
        :alt="data.alt || ''"
      >
        <div class="action">
          <div class="external-link">
            <LinkButton
              external
              variant="outline"
              size="lg"
              :url="data.button_url"
              >{{ data.button_text }}</LinkButton
            >
          </div>
        </div>
        <template v-slot:imageExt>
          <div class="project-logo">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              xmlns:xlink="http://www.w3.org/1999/xlink"
              viewBox="0 0 362.83 102.05"
            >
              <defs>
                <clipPath id="d">
                  <rect
                    class="f"
                    x="29.94"
                    y="69.57"
                    width="216.89"
                    height="16.69"
                  />
                </clipPath>
                <clipPath id="e">
                  <rect
                    class="f"
                    x="29.94"
                    y="18.9"
                    width="216.74"
                    height="43.26"
                  />
                </clipPath>
              </defs>
              <g id="a" />
              <g id="b">
                <g id="c">
                  <path
                    class="h"
                    d="M350.75,83.95l.03-.03c.34-.4,.67-.82,1-1.23,.13-.17,.26-.33,.39-.5,.29-.37,.57-.74,.84-1.12,.18-.24,.35-.49,.52-.73,.23-.33,.47-.66,.7-1,.38-.57,.76-1.15,1.12-1.74,.11-.17,.21-.35,.31-.53,.29-.49,.58-.99,.86-1.49,.1-.18,.19-.36,.29-.54,.29-.55,.58-1.11,.85-1.67,.06-.11,.11-.23,.17-.34,3.2-6.66,5-14.12,5-21.99s-1.79-15.28-4.97-21.93c-.08-.17-.17-.34-.25-.51-.24-.49-.49-.98-.75-1.47-.13-.24-.25-.47-.38-.71-.23-.41-.47-.82-.71-1.23-.27-.46-.55-.91-.83-1.36-.23-.36-.45-.71-.68-1.06-.28-.42-.57-.82-.86-1.23-.12-.16-.23-.33-.35-.49-.31-.43-.63-.86-.96-1.28-.08-.1-.16-.21-.24-.31C342.53,7.62,328.06,0,311.81,0H4.2C1.45,16.61,0,33.66,0,51.04s1.44,34.4,4.19,51.01H311.81c15.62,0,29.59-7.04,38.94-18.1Z"
                  />
                  <g class="j">
                    <path
                      class="g"
                      d="M241.48,71.28h1.26c1.32,0,2.05,.12,2.35,.51,.33,.39,.69,1.5,.9,2.17h.84v-4.06h-13.26v4.06h.87c.21-.66,.51-1.77,.84-2.17,.33-.39,1.05-.51,2.38-.51h1.26v11.34c0,1.11-.06,1.78-.45,2.01-.39,.24-1.02,.45-1.71,.63v.75h6.89v-.75c-.63-.18-1.32-.39-1.71-.63-.42-.27-.45-.9-.45-2.01v-11.34Zm-16.9,14.74h6.95v-.75c-.63-.18-1.32-.39-1.71-.63-.42-.27-.48-.9-.48-2.01v-9.35c0-1.11,.06-1.74,.48-2.01,.39-.24,.99-.45,1.65-.66v-.69h-6.86v.69c.69,.21,1.29,.42,1.68,.66,.39,.24,.48,.9,.48,2.01v9.35c0,1.11-.09,1.78-.48,2.01-.39,.24-1.02,.45-1.71,.63v.75Zm-14.95,0h11.91l.33-4.12-.96-.09c-.24,.87-.81,1.98-1.14,2.32-.36,.36-.9,.57-2.26,.57h-1.53c-1.35,0-1.65-.75-1.65-2.32v-3.97h1.65c1.35,0,2.01,.06,2.32,.45,.3,.36,.45,.9,.69,1.56h.72v-5.38h-.72c-.24,.66-.39,1.2-.69,1.56-.3,.36-.96,.45-2.32,.45h-1.65v-5.83h2.56c1.35,0,2.05,.09,2.38,.45,.33,.42,.63,1.29,.84,1.95h.84v-3.88c-.27,.06-.9,.15-1.23,.15h-10.04v.69c.69,.21,1.29,.42,1.68,.66s.48,.9,.48,2.01v9.35c0,1.11-.09,1.78-.48,2.01-.39,.24-1.02,.45-1.71,.63v.75Zm-7.61-2.74c.66,.96,.99,1.41,1.14,2.74h4.87v-.75c-.81-.3-1.23-.48-1.74-.84-.48-.42-1.11-1.05-1.62-1.77l-4.06-5.83,4.63-4.81c.63-.66,1.65-1.11,2.44-1.41v-.69h-5.71v.69c.69,.18,1.32,.36,1.32,.81,0,.27-.24,.57-.54,.93l-4.72,5.2,4,5.74Zm-9.29,2.74h6.89v-.75c-.63-.18-1.32-.39-1.71-.63-.42-.27-.48-.9-.48-2.01v-9.41c0-1.11,.06-1.68,.48-1.95,.39-.24,.99-.45,1.65-.66v-.69h-6.8v.69c.69,.21,1.29,.42,1.68,.66,.39,.24,.48,.84,.48,1.95v9.41c0,1.11-.09,1.78-.48,2.01-.39,.24-1.02,.45-1.71,.63v.75Zm-6.44-2.74c0,1.32-1.92,1.47-3.22,1.47-3.25,0-5.32-2.71-5.32-7.43,0-3.91,2.01-6.13,5.62-6.13,2.71,0,3.73,.63,4.42,2.98h.84v-3.82c-1.29-.42-3.19-.69-4.66-.69-5.17,0-9.11,2.68-9.11,8.63,0,5.41,3.58,7.97,8.03,7.97,1.77,0,4.03-.3,5.83-.9v-3.4c0-1.8,.03-1.86,1.23-2.41v-.72h-6.26v.72c.84,.24,1.77,.45,2.17,.78,.36,.3,.42,.84,.42,2.11v.84Zm-20.81,2.74h6.95v-.75c-.63-.18-1.32-.39-1.71-.63-.42-.27-.48-.9-.48-2.01v-9.35c0-1.11,.06-1.74,.48-2.01,.39-.24,.99-.45,1.65-.66v-.69h-6.86v.69c.69,.21,1.29,.42,1.68,.66,.39,.24,.48,.9,.48,2.01v9.35c0,1.11-.09,1.78-.48,2.01-.39,.24-1.02,.45-1.71,.63v.75Zm-7.4-14.74h1.26c1.32,0,2.05,.12,2.35,.51,.33,.39,.69,1.5,.9,2.17h.84v-4.06h-13.26v4.06h.87c.21-.66,.51-1.77,.84-2.17,.33-.39,1.05-.51,2.38-.51h1.26v11.34c0,1.11-.06,1.78-.45,2.01-.39,.24-1.02,.45-1.71,.63v.75h6.89v-.75c-.63-.18-1.32-.39-1.71-.63-.42-.27-.45-.9-.45-2.01v-11.34Zm-15.64,1.92c0-1.11,.06-1.68,.48-1.95,.39-.24,.99-.45,1.65-.66v-.69h-6.8v.69c.69,.21,1.29,.42,1.68,.66,.39,.24,.48,.84,.48,1.95v9.41c0,1.11-.09,1.78-.48,2.01-.39,.24-1.02,.45-1.71,.63v.75h11.64l.33-4.36-.9-.09c-.27,.87-.81,2.17-1.14,2.5-.39,.36-.93,.63-2.29,.63h-1.29c-1.32,0-1.65-1.05-1.65-2.32v-9.17Zm-14.68-.57l2.29,6.59h-4.63l2.35-6.59Zm-7.91,13.38h5.83v-.75c-.69-.24-1.86-.51-1.86-1.17,0-.39,.3-1.11,.57-1.89l.48-1.38h5.74l.48,1.38c.3,.81,.57,1.53,.57,1.92,0,.6-1.11,.93-1.8,1.14v.75h6.44v-.75c-.69-.24-1.11-.45-1.44-.75-.33-.36-.57-.81-.9-1.71l-4.81-13.23h-1.98l-4.9,13.23c-.33,.9-.54,1.35-.9,1.71-.33,.3-.78,.51-1.5,.75v.75Zm-8.33,0h6.89v-.75c-.63-.18-1.32-.39-1.71-.63-.42-.27-.48-.9-.48-2.01v-9.35c0-1.11,.06-1.74,.48-2.01,.39-.24,.99-.45,1.65-.66v-.69h-6.8v.69c.69,.21,1.29,.42,1.68,.66,.39,.24,.48,.9,.48,2.01v3.61h-7.46v-3.61c0-1.11,.06-1.74,.48-2.01,.39-.24,.96-.45,1.62-.66v-.69h-6.77v.69c.69,.21,1.29,.42,1.68,.66,.39,.24,.48,.9,.48,2.01v9.35c0,1.11-.09,1.78-.48,2.01-.39,.24-1.02,.45-1.71,.63v.75h6.89v-.75c-.63-.18-1.32-.39-1.71-.63-.42-.27-.48-.9-.48-2.01v-3.97h7.46v3.97c0,1.11-.09,1.78-.48,2.01-.39,.24-1.02,.45-1.71,.63v.75Zm-19.73,0h6.89v-.75c-.63-.18-1.32-.39-1.71-.63-.42-.27-.48-.9-.48-2.01v-9.35c0-1.11,.06-1.74,.48-2.01,.39-.24,.99-.45,1.65-.66v-.69h-6.8v.69c.69,.21,1.29,.42,1.68,.66,.39,.24,.48,.9,.48,2.01v3.61h-7.46v-3.61c0-1.11,.06-1.74,.48-2.01,.39-.24,.96-.45,1.62-.66v-.69h-6.77v.69c.69,.21,1.29,.42,1.68,.66,.39,.24,.48,.9,.48,2.01v9.35c0,1.11-.09,1.78-.48,2.01-.39,.24-1.02,.45-1.71,.63v.75h6.89v-.75c-.63-.18-1.32-.39-1.71-.63-.42-.27-.48-.9-.48-2.01v-3.97h7.46v3.97c0,1.11-.09,1.78-.48,2.01-.39,.24-1.02,.45-1.71,.63v.75Zm-13.95-4.75c-.78,2.65-1.29,3.46-4.42,3.46s-5.17-2.74-5.17-7.46c0-3.82,1.98-6.08,5.5-6.08,2.71,0,3.46,.72,4.12,2.98h.84v-3.82c-1.23-.42-2.86-.69-4.24-.69-5.08,0-9.14,2.53-9.14,8.57,0,5.32,3.49,8.03,7.91,8.03,1.74,0,3.52-.21,5.26-.9l.24-4-.9-.09Zm-22.19-8.63l2.29,6.59h-4.63l2.35-6.59Zm-7.91,13.38h5.83v-.75c-.69-.24-1.86-.51-1.86-1.17,0-.39,.3-1.11,.57-1.89l.48-1.38h5.74l.48,1.38c.3,.81,.57,1.53,.57,1.92,0,.6-1.11,.93-1.8,1.14v.75h6.44v-.75c-.69-.24-1.11-.45-1.44-.75-.33-.36-.57-.81-.9-1.71l-4.81-13.23h-1.99l-4.9,13.23c-.33,.9-.54,1.35-.9,1.71-.33,.3-.78,.51-1.5,.75v.75Zm-17.8,0h6.22v-.75c-.63-.18-1.32-.39-1.71-.63-.42-.27-.48-.9-.48-2.01v-9.77l8.93,13.17h1.74v-12.72c0-1.11,.09-1.77,.48-2.02,.39-.24,1.02-.45,1.71-.63v-.75h-6.22v.75c.63,.18,1.32,.39,1.71,.63,.42,.27,.48,.9,.48,2.02v8.12l-7.79-11.52h-5.08v.75c.63,.18,1.32,.39,1.71,.63,.42,.27,.48,.9,.48,2.02v9.32c0,1.11-.09,1.78-.48,2.01-.39,.24-1.02,.45-1.71,.63v.75Z"
                    />
                  </g>
                  <g class="i">
                    <path
                      class="g"
                      d="M49.67,29.08c0,5.73-2.86,7.64-7.83,7.64-.64,0-1.27,0-1.97-.06v-14.57c.57-.06,1.15-.06,1.59-.06,4.58,0,8.21,1.65,8.21,7.06m5.79-.76c0-3.88-2.61-7.19-5.98-8.21-1.78-.57-4.71-.76-7.45-.76-1.65,0-7.06,.13-12.03,.38v1.46c1.46,.45,2.74,.89,3.56,1.4,.83,.51,1.02,1.91,1.02,4.26v19.6c0,2.35-.19,3.75-1.02,4.26-.83,.51-2.16,.95-3.63,1.34v1.59h15.21v-1.59c-1.46-.38-3.44-.83-4.26-1.34s-1.02-1.91-1.02-4.26v-6.62c.7,.06,1.34,.06,1.97,.06,7.51,0,13.62-3.95,13.62-11.58"
                    />
                    <path
                      class="g"
                      d="M72.9,36.01h-2.86v-13.94c.64-.06,1.27-.06,1.85-.06,4.26,0,7.7,1.34,7.7,6.49,0,4.71-1.65,7.51-6.68,7.51m18.2,17.63v-1.4c-1.65-.64-2.55-1.02-3.63-1.85-1.02-.83-2.16-2.23-3.12-3.88l-5.28-8.91c4.58-2.29,6.36-6.55,6.36-10.05,0-4.14-2.8-6.55-5.79-7.51-1.85-.57-4.9-.7-7.51-.7-1.65,0-7,.13-11.96,.38v1.46c1.46,.45,2.74,.89,3.56,1.4,.83,.51,1.02,1.91,1.02,4.26v19.6c0,2.35-.19,3.75-1.02,4.26s-2.16,.95-3.63,1.34v1.59h14.57v-1.59c-1.46-.38-2.8-.83-3.63-1.34-.83-.51-1.02-1.91-1.02-4.26v-7.89h3.63l5.35,9.29c1.21,2.1,2.1,3.63,2.61,5.79h9.48Z"
                    />
                    <path
                      class="g"
                      d="M120.94,37.86c0,9.35-4.14,12.85-10.18,12.85s-10.88-4.84-10.88-15.59c0-7.7,3.37-12.66,10.31-12.66,7.57,0,10.75,6.81,10.75,15.4m6.11-2.1c0-9.61-5.92-16.86-15.59-16.86-10.75,0-17.63,7.64-17.63,18.52,0,10.25,6.3,16.86,15.72,16.86,10.82,0,17.5-7.06,17.5-18.52"
                    />
                    <path
                      class="g"
                      d="M142.71,26.53c0-2.35,.13-3.56,1.02-4.14,.83-.51,2.1-.95,3.5-1.4v-1.46h-14.57v1.46c1.46,.45,2.74,.89,3.56,1.4,.83,.51,1.02,1.78,1.02,4.14v23.02c0,5.03-1.97,7.45-5.22,10.37l1.65,2.23c6.17-3.82,9.04-6.75,9.04-13.94V26.53Z"
                    />
                    <path
                      class="g"
                      d="M153.01,53.64h25.2l.7-8.72-2.04-.19c-.51,1.85-1.72,4.2-2.42,4.9-.76,.76-1.91,1.21-4.77,1.21h-3.25c-2.86,0-3.5-1.59-3.5-4.9v-8.4h3.5c2.86,0,4.26,.13,4.9,.95,.64,.76,.95,1.91,1.46,3.31h1.53v-11.39h-1.53c-.51,1.4-.83,2.55-1.46,3.31-.64,.76-2.04,.95-4.9,.95h-3.5v-12.35h5.41c2.86,0,4.33,.19,5.03,.95,.7,.89,1.34,2.74,1.78,4.14h1.78v-8.21c-.57,.13-1.91,.32-2.61,.32h-21.25v1.46c1.46,.45,2.74,.89,3.56,1.4,.83,.51,1.02,1.91,1.02,4.26v19.79c0,2.35-.19,3.75-1.02,4.26-.83,.51-2.16,.95-3.63,1.34v1.59Z"
                    />
                    <path
                      class="g"
                      d="M184.64,53.64h14.57v-1.59c-1.34-.38-2.8-.83-3.63-1.34-.89-.57-1.02-1.91-1.02-4.26V26.53c0-2.35,.13-3.56,1.02-4.14,.83-.51,2.1-.95,3.5-1.4v-1.46h-14.38v1.46c1.46,.45,2.74,.89,3.56,1.4,.83,.51,1.02,1.78,1.02,4.14v19.92c0,2.35-.19,3.75-1.02,4.26-.83,.51-2.16,.95-3.63,1.34v1.59Z"
                    />
                    <path
                      class="g"
                      d="M204.3,47.85c1.4,2.04,2.1,2.99,2.42,5.79h10.31v-1.59c-1.72-.64-2.61-1.02-3.69-1.78-1.02-.89-2.35-2.23-3.44-3.75l-8.59-12.35,9.8-10.18c1.34-1.4,3.5-2.35,5.15-2.99v-1.46h-12.09v1.46c1.46,.38,2.8,.76,2.8,1.72,0,.57-.51,1.21-1.15,1.97l-9.99,11.01,8.46,12.15Z"
                    />
                    <path
                      class="g"
                      d="M235.36,22.46h2.67c2.8,0,4.33,.25,4.96,1.08,.7,.83,1.46,3.18,1.91,4.58h1.78v-8.59h-28.06v8.59h1.85c.45-1.4,1.08-3.75,1.78-4.58s2.23-1.08,5.03-1.08h2.67v23.99c0,2.35-.13,3.75-.95,4.26-.83,.51-2.16,.95-3.63,1.34v1.59h14.57v-1.59c-1.34-.38-2.8-.83-3.63-1.34-.89-.57-.95-1.91-.95-4.26V22.46Z"
                    />
                  </g>
                  <g>
                    <path
                      class="g"
                      d="M311.81,7.97c-23.74,0-43.05,19.31-43.05,43.05s19.31,43.05,43.05,43.05,43.05-19.31,43.05-43.05-19.31-43.05-43.05-43.05Zm0,78.49c-19.55,0-35.44-15.89-35.44-35.44s15.9-35.44,35.44-35.44,35.44,15.9,35.44,35.44-15.9,35.44-35.44,35.44Z"
                    />
                    <path
                      class="g"
                      d="M319.17,70.92l-12.29-22.54-3.06-6.89,.44,6.83v22.6h-8.36V31.13h10.29l10.42,19.19,3.12,7.18-.37-6.89V31.13h8.36v39.79h-8.55Z"
                    />
                  </g>
                </g>
              </g>
            </svg>
          </div>
        </template>
      </content-with-image>
    </section>
    <section
      v-if="data.accordion_title && data.accordion_entries?.length"
      class="content-section"
    >
      <div class="content-section-wrapper">
        <div class="accordion-title">
          <h2>{{ data.accordion_title }}</h2>
        </div>
        <accordion>
          <accordion-entry
            v-for="entry of data.accordion_entries"
            :key="entry.title"
            :title="entry.title"
            :subtitle="entry.subtitle"
            :content="entry.content"
          />
        </accordion>
        <div class="accordion-button">
          <LinkButton
            v-if="data.accordion_button_text && data.accordion_button_url"
            external
            :url="data.accordion_button_url"
            >{{ data.accordion_button_text }}</LinkButton
          >
        </div>
      </div>
    </section>
    <section
      class="content-section cards-section"
      v-if="orgas.content && orgas.content?.length"
    >
      <div class="cards content-section-wrapper">
        <div class="title-wrapper">
          <h2 class="slider-title">{{ data.title_slider }}</h2>
        </div>
      </div>
    </section>
    <div class="carousel-wrapper" v-if="orgas && orgas?.length">
      <Carousel
        :items="orgas"
        :component="OrgaActivitySlide"
        framedCards
      ></Carousel>
    </div>
  </div>
</template>

<script setup>
import OrgaActivitySlide from '../components/OrgaActivitySlide.vue';

const { t } = useI18n();
const { $i18n } = useNuxtApp();
const { getItems, getSingletonItem } = useDirectusItems();
const config = useRuntimeConfig();

const orgas = ref([]);

const { data, pending, error, refresh } = await useAsyncData(
  'projectN',
  async () => {
    const currentLocale = $i18n.locales.value.find(
      (i) => i.code === $i18n.locale.value
    );

    let projectNTranslations = [];
    let projectN = {};
    let parentNav = {};
    let currentNav = {};
    try {
      const nav = await getItems({
        collection: 'navigation_translations',
        params: {
          filter: {
            languages_code: currentLocale.iso,
          },
          fields: ['*', '**'],
        },
      });
      const navItems = (nav[0] && nav[0].nav_item) || [];
      parentNav = navItems.find((i) => i.key === 'activities');
      currentNav = parentNav?.submenu.find(
        (s) => s.target === 'projekt-nachhaltigkeit'
      );

      projectN = await getSingletonItem({
        collection: 'projekt_nachhaltigkeit',
      });
      projectNTranslations = await getItems({
        collection: 'projekt_nachhaltigkeit_translations',
        params: {
          filter: {
            languages_code: currentLocale.iso, //'en-US'
          },
        },
      });
    } catch (error) {
      console.log('DIRECTUS Fetch Error', error);
    }

    return {
      ...projectN,
      ...projectNTranslations[0],
      currentNav,
      parentNav,
    };
  }
);

onMounted(async () => {
  try {
    const response = await $fetch(
      config.public.privateApiUrl +
        '/organisations?projectSustainabilityWinner=true&size=60'
    );
    if (response) {
      orgas.value = response.content;
    }
  } catch (e) {
    orgas.value = [];
  }
});

const title = data.value.title;

useHead({
  title: title,
  meta: [
    {
      hid: 'og-title',
      property: 'og:title',
      content: title + ' - ' + t('page.title'),
    },
  ],
});
</script>

<style scoped lang="scss">
/* For project n svg */
.f {
  fill: none;
}

.g {
  fill: #fff;
}

.h {
  fill: #c8d400;
}

.i {
  clip-path: url(#e);
}

.j {
  clip-path: url(#d);
}

/* For project n svg - end */

.content-section-wrapper {
  width: 100%;
}

.breadcrumbs-section {
  padding-top: 20px;
  padding-bottom: 20px;

  .breadcrumbs {
    flex-grow: 1;
    display: flex;
    align-items: center;
    flex-wrap: wrap;

    .arrow {
      padding: 0 16px;
    }

    .current {
      font-weight: 600;
    }
  }
}

.page-hero {
  position: relative;
  padding-top: 20px;

  .project-logo {
    position: absolute;
    right: 0;
    bottom: -44px;
    width: 50%;
    height: 88px;

    img {
      width: 100%;
      height: 100%;
      object-fit: contain;
    }
  }

  .action {
    margin-top: 40px;

    .external-link {
      width: fit-content;
    }
  }
}

.cards-section {
  padding: 46px 0 0 0;

  .cards {
    flex-grow: 1;
  }

  .title-wrapper {
    max-width: 500px;
  }
}

.accordion-title {
  margin-bottom: 40px;
}

.accordion-button {
  width: fit-content;
  margin: auto;
  margin-top: 64px;
}

.carousel-wrapper {
  padding: 24px 0 100px;
}
</style>
