  ...
  <div class="page">
    ...
    <section class="content-section">
      <div class="content-section-wrapper sidebar-layout">
        ...
        <div class="content">
          ...
          <section class="nav-section small-gap">
            <div class="inner-section">
              <div class="inner-layout-vertical">
                ...
                <div
                  class="html no-margin"
                  v-if="data.actions_description"
                  v-html="data.actions_description"
                ></div>
              </div>
            </div>
          </section>
          ...
          >
            <div class="inner-section">
              <ParticipateActivity
                ...
                :image="action.image"
                :logos="action.cooperations"
                :logoLabel="action.cooperations_title"
                :sliderTitle="action.image_slider_title"
                :slider="!!action.slides?.length"
                :button="
                  action.series_popup_active ? action.series_popup_button : null
                "
                :show-logos="action.slider_active"
                :show-cooperations="action.cooperations_active"
                :videoSectionActive="action.video_series_active"
                :videoSectionVideosTitle="action.video_series_videos_title"
                :videoSectionImage="action.video_series_image"
                :videoSectionTitle="action.video_series_title"
          ...
          <section
            class="nav-section knowledge-section"
            :class="{ 'knowledge-section-space': !data.highlights_title }"
            id="wissenssammlung"
            data-track-menu
          >
            <div class="inner-section">
              <div class="inner-layout-vertical">
                ...
                <div>
                  <h3 class="list-title" v-if="data.knowledge_list_title">
                    {{ data.knowledge_list_title }}
                  </h3>
                </div>
                <div class="list">
                  <div
                    class="list-item"
                    v-for="listItem in data.knowledge_list"
                    :key="listItem"
                  >
                    <div class="icon">
                      <svg
                ...
                <div
                  class="html no-margin"
                  v-if="data.knowledge_content"
                  v-html="data.knowledge_content"
                ></div>

                <div
                  class="knowledge-accordion"
                  v-if="
                    data.knowledge_accordion && data.knowledge_accordion.length
                  "
                >
                  <accordion>
                    <accordion-entry
                      v-for="entry of data.knowledge_accordion"
                      :key="entry.title"
                      :title="entry.title"
          ...
          <section
            class="nav-section highlights-section"
            id="highlights"
            data-track-menu
          >
            <div class="inner-section">
    ...
    <template>
      <div>
        <PopUp
          v-for="action in data?.actions"
          :open="openPopup === action.menu_slug"
          @popupClosed="handleClosePopup"
        >
          <PopUpContent
            :title="action.series_popup_title"
            :content="wasSubscriptionSuccess ? '' : action.series_popup_content"
          >
            <SubscriptionForm
              :endpoint="'action-page/form'"
              :options="action.series_popup_stations"
              :lang="$i18n.locale.value"
              :stations="
                data.stations && data.stations[action.series_popup_key]
              "
              :form-key="action.series_popup_key"
...
const { data, pending, error, refresh } = await useAsyncData(
  ...
  async () => {
    ...
    try {
      ...
      const focusTopicPromise = getSingletonItem({
        collection: 'focus_topic_page',
      });
      ...
      const [focusTopicResp, focusTopicTranslationsRes] = await Promise.all([
        focusTopicPromise,
      ...
      if (focusTopic.actions && focusTopic.actions.length) {
        actionTranslations = await getItems({
          ...
          params: {
            ...
            fields: ['*', '**', 'cooperations.*', 'slides.*', 'videos.*'],
          },
        });
        const allActionTranslationsUngrouped = await getItems({
          ...
          params: {
            ...
            fields: [
              'series_popup_key',
              'series_popup_stations',
              'languages_code',
            ],
          },
        });
        allActionTranslations = allActionTranslationsUngrouped.reduce(
          (acc, entry) => {
            if (!acc[entry.series_popup_key]) {
              acc[entry.series_popup_key] = {};
            }
            acc[entry.series_popup_key][entry.languages_code] =
              entry.series_popup_stations;
            return acc;
          },
          {}
        );
      ...
      focusTopicTranslations =
        focusTopicTranslationsRes && focusTopicTranslationsRes[0];
      ...
      const sliderPromises = actionTranslations.map(async (a) => {
        ...
        a.slides = slides.map((slide) => {
          const trans = slide.translations.find(
            (t) => t.languages_code === currentLocale.iso
          );
          return {
            ...trans,
            image: slide.image,
      ...
      const videoPromieses = actionTranslations.map(async (a) => {
        const ids = a.videos.map((s) => s.focus_topic_videos_id) || [];
        if (!ids.length) {
          return [];
        }

        const videos = await getItems({
          collection: 'focus_topic_videos',
          params: {
            filter: {
              id: {
                _in: ids,
        ...
        a.videos = videos.map((video) => {
          const trans = video.translations.find(
            (t) => t.languages_code === currentLocale.iso
          );
  ...
  try {
    ...
    if (ids && ids.length) {
      const promises = ids.map(async (id) => {
        try {
          return await $fetch(
            config.public.privateApiUrl + '/activities/' + id
          );
        } catch (error) {
          return null;
        }
