  ...
  <div :id="id" class="menu menu-main" :class="{ footer: inFooter }">
    ...
    <template v-if="inFooter">
      <template v-for="(item, index) of items">
        <MenuEntry v-if="!item?.submenu" inFooter :url="getItemUrl(item, localePath)" :external="!item.lp" groupLine
          :isNew="item.new">{{
            item.name }}
        </MenuEntry>
        <template v-else>
          <div :class="{ group: index < items.length - 1 }">
            <MenuEntry v-for="subitem of item?.submenu" inFooter :url="getItemUrl(subitem, localePath)" :isNew="item.new"
    ...
    <template v-else>
      <div v-for="item of items">
        <MenuEntry v-if="!item?.submenu" :url="getItemUrl(item, localePath)" :external="!item.lp" groupLine
          :isNew="item.new" @menuClicked="closeSidebar">
          {{
            item.name }}
        </MenuEntry>

        <template v-else>
          <DropdownMenuEntry :isNew="item.new" :isActive="dropdownActive(item.key)" :title="item.name" class="desktop"
            :open="open === item.key" @openMenu="openDropdownMenu(item.key)"
            @closeMenu="() => closeDropdownMenu(item.key)">

            <MenuEntry v-for="subitem of item?.submenu" :url="getItemUrl(subitem, localePath)" enforceUnderlineAlways blue
              @menuClicked="closeDropdownMenu(item.key)">
              {{ subitem.name }}
            </MenuEntry>

          </DropdownMenuEntry>
          <CollapsableMenuEntry :isNew="item.new" :isActive="dropdownActive(item.key)" :title="item.name" class="mobile">
            <MenuEntry v-for="subitem of item?.submenu" :url="getItemUrl(subitem, localePath)" enforceUnderlineAlways blue
