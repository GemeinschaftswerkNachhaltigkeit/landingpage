<template>
  <div
    ref="carousel"
    class="carousel"
    :class="{ noTopPadding: noTopPadding }"
  ...
  >
    ...
    >
      <div class="slide" v-for="(item, index) in items" :key="item.id">
        <component :is="component" v-bind="getProps(item)"> </component>
      </div>
    </Flicking>
    <div
      class="actions"
      :class="{ actionsRight: actionsRight, framedCards: framedCards }"
      v-if="items?.length > 1"
    >
        ...
        >
          ...
          >
            <rect
              width="17.1429"
              height="17.1429"
              transform="matrix(-1 0 0 1 17.5723 0.428589)"
              fill="#D9D9D9"
            />
          </mask>
