<template>
  <div class="select-wrapper" v-click-outside="closeOptions">
    ...
    <div class="options-wrapper">
      ...
      <div v-for="option in options" :key="option.key" class="option" >
          <input :id="'option' + option.key" type="checkbox" v-model="selectedSdgs" :value="option.key" @change="goalToggledHandler"> 
...
export default {
  ...
  computed: {
    values() {
      if (!this.valuesMapper) {
        ...
        return this.valuesMapper(this.value);
      }
    },
    count() {
  ...
  methods: {
    ...
    goalToggledHandler() {
      this.$emit("input", this.selectedSdgs);
    },
  },
};
</script>
