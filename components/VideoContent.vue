<template>
  <div class="video-wrapper" v-if="heroImageLandscape">
    <video
      ...
      :data-landscape-img="
        getImgSrcSet(heroImageLandscape, 'webp', 'landscape')
      "
      :data-portrait-img="getImgSrcSet(heroImageLandscape, 'webp', 'portrait')"
      :poster="getImgSrcSet(heroImageLandscape, 'webp', 'landscape')"
    >
...
function getImgSrcSet(imageId, type, orientation) {
  if (orientation == 'portrait') {
    return $imageURL(imageId, { format: type, width: 1440 });
  } else {
    return $imageURL(imageId, { format: type, width: 1920 });
  }
}
